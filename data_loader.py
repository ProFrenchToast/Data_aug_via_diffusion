...
def env_action_to_json_action(env_action):
    # first find all the keys pressed and add the matching string to the array
    keyboard_keys = []
    for key, button in KEYBOARD_BUTTON_MAPPING.items():
        if env_action[button] == 1:
            keyboard_keys.append(key)

    # now add find the camera movement
    # also need to reverse the scaling hence the divide 
    dy = env_action["camera"][0] / CAMERA_SCALER
    dx = env_action["camera"][1] / CAMERA_SCALER

    # now set the mouse buttons
    mouse_buttons = []
    if env_action["attack"] == 1:
        mouse_buttons.append(0)
    if env_action["use"] == 1:
        mouse_buttons.append(1)
    if env_action["pickItem"] == 1:
        mouse_buttons.append(2)

    # finally create the full json object to be saved
    json_action = json_NOOP.copy()
    json_action["keyboard"]["keys"] = keyboard_keys
    json_action["mouse"]["dy"] = dy
    json_action["mouse"]["dx"] = dx
    json_action["mouse"]["buttons"] = mouse_buttons
    return json_action
...
def load_demonstration(trajectory_id, video_path, json_path):
    """
...
class Data_Loader:
    ...
    def __init__(self, dataset_dir, n_workers=8, batch_size=8, n_epochs=1, max_queue_size=16):
        ...
        for unique_id in unique_ids:
            ...
            assert json_path is not None, f"action file missing for sample: {unique_id}"
